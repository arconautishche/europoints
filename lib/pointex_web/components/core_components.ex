defmodule PointexWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed hidden top-2 right-2 w-80 sm:w-96 z-50 rounded-lg px-4 py-2 shadow-md ring-1",
        @kind == :info && "bg-emerald-50 text-emerald-800 ring-emerald-500 fill-cyan-900",
        @kind == :error && "bg-rose-50 p-3 text-rose-900 shadow-md ring-rose-800/25 fill-rose-900"
      ]}
      {@rest}
    >
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          "mt-1 block w-full rounded-lg border-zinc-300 py-[7px] px-[11px]",
  ...
  def error(assigns) do
    ~H"""
    <p class="phx-no-feedback:hidden mt-1 flex gap-3 text-sm leading-6 text-rose-600">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PointexWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PointexWeb.Gettext, "errors", msg, opts)
    end
  end
